import { FrequencyResponse } from "../qrwc-graphic-equalizer-component";
/**
 ...
 */
export class GraphicEQResponseCalculator {
  /**
  ...
  private bandwidthToQ(bandwidth: number): number {
    return Math.sqrt(Math.pow(2, bandwidth)) / (Math.pow(2, bandwidth) - 1);
  }

  private qToBandwidth(Q: number): number {
  ...
  sumEQBands(
    ...
    bands.forEach((band) => {
      const { realResponse, imagResponse } = this.calculateBiquadResponse(
        f,
        band,
        quality,
        sampleRate
  ...
  public calculateFilterResponse(
    ...
    var response = this.frequencies.map((frequency) => {
      const { magnitude, phase } = this.sumEQBands(frequency, bands, quality);
      return {
        Frequency: frequency,
        Magnitude: magnitude,
        Phase: phase,
  ...
  public calculateFilterResponseFromBandwidth(
    bands: GraphicEQBand[],
    bandwidth: number
  ): FrequencyResponse[] {
    return this.calculateFilterResponse(bands, this.bandwidthToQ(bandwidth));
  }
}
